...
export default function LoginScreen({ navigation }) {
  ...
  const handleResumeSubscription = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await api.get(`/stripe/subscription/resume?email=${email || storedEmail}`);
      handleLogin(storedEmail, storedPassword);
    } catch (error) {
        ...
        <View style={styles.modalCenteredView}>
          <View style={styles.modalView}>
            ...
            <View style={{ flexDirection: 'row', gap: 20 }}>
              ...
              <TouchableOpacity style={styles.button} onPress={handleResumeSubscription} disabled={isLoading}>
