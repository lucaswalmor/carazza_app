...
const GPSNavigatorScreen = () => {
  const mapRef = useRef(null);
  const [cep, setCep] = useState("38080615");
  const [nomeRota, setNomeRota] = useState("");
  const [routes, setRoutes] = useState([]);
  const [numero, setNumero] = useState("91");
  ...
  const calculateRoute = async () => {
    ...
    try {
      const url = `https://us1.locationiq.com/v1/directions/driving/${location.longitude},${location.latitude};${destinationCoords.longitude},${destinationCoords.latitude}?key=${LOCATIONIQ_API_KEY}&steps=true&geometries=polyline&overview=full`
